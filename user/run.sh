...
append_database "clientprefs" "$DB_CLIENTPREFS_DRIVER" "$DB_CLIENTPREFS_HOST" "$DB_CLIENTPREFS_PORT" "$DB_CLIENTPREFS_DATABASE" "$DB_CLIENTPREFS_USER" "$DB_CLIENTPREFS_PASSWORD"

install_layer "MovementAPI"
install_layer "GOKZ"
append_database "gokz" "$DB_GOKZ_DRIVER" "$DB_GOKZ_HOST" "$DB_GOKZ_PORT" "$DB_GOKZ_DATABASE" "$DB_GOKZ_USER" "$DB_GOKZ_PASSWORD"
echo $KZ_APIKEY > "$server_dir/csgo/cfg/sourcemod/globalapi-key.cfg"

install_layer "MiscPlugins"
append_database "more-stats" "$DB_MORESTATS_DRIVER" "$DB_MORESTATS_HOST" "$DB_MORESTATS_PORT" "$DB_MORESTATS_DATABASE" "$DB_MORESTATS_USER" "$DB_MORESTATS_PASSWORD"
append_database "no_dupe_account" "$DB_NODUPE_DRIVER" "$DB_NODUPE_HOST" "$DB_NODUPE_PORT" "$DB_NODUPE_DATABASE" "$DB_NODUPE_USER" "$DB_NODUPE_PASSWORD"
sed -i -E "s/(\"FollowCSGOServerGuidelines\"[[:space:]]+)\"[^\"]+\"/\1\"no\"/" "$server_dir/csgo/addons/sourcemod/configs/core.cfg"

install_layer "SBPP"
append_database "sourcebans" "$DB_SBPP_DRIVER" "$DB_SBPP_HOST" "$DB_SBPP_PORT" "$DB_SBPP_DATABASE" "$DB_SBPP_USER" "$DB_SBPP_PASSWORD"
